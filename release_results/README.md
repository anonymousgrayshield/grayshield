...
- camera-ready RQ plots generated by `scripts/generate_camera_ready_figures.py`
...
Compact camera-ready RQ figures and the appendix defense-trade-off plot can
be regenerated from the same bundled artifacts with:

```bash
python scripts/generate_camera_ready_figures.py
```

The wrapper writes its outputs to `release_results/camera_ready_figures/`.
These compact camera-ready variants complement the original per-RQ figures in
`rq1/`, `rq2/`, `rq3/`, and `rq4/`; they do not replace those files unless the
manuscript is updated to reference the new filenames.
